...
| POST | `/inventory/equipment/{id}/photos` | `{attachmentIds:[...]}` — a batch, because the plate and the damage are photographed in the same minute and one request per picture re-opens the row per picture. Same rule as PUT above; adding to a row that already carries pictures re-opens it from the field, adding the first ones does not |
| DELETE | `/inventory/equipment/{id}/photos/{photoId}` | takes one picture off and discards the file with it. Named by the photograph's own row, so nothing can unpick a file from a machine it never belonged to. Removing from the field always re-opens a decided row |
