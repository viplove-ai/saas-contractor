 ...
 * or screenshot that proves a payment went out.
 *
 * <p>One component for both because it is one problem twice, and the two halves of it are
 * worth stating.</p>
 *
 * <p><b>Compression happens here</b>, when the file is picked, rather than at upload time. Two
 * reasons, and the second is the one that matters. Somebody who takes a photograph and
 ...
 *
 * <p><b>What was picked is shown as a picture.</b> A file name is not evidence: the thumb over
 * the lens, the challan photographed instead of the bill, the wrong screenshot out of a roll
 * of forty — all of them look identical as {@code IMG_20260812_104533.jpg}, and all of them
 * are obvious as a picture. Caught here it costs another photograph; caught by an approver it
...
export function EvidencePhotoField({
  ...
  /** What the empty control offers to do. "Photograph the bill", "Attach the receipt". */
  label: string;
  /** The same offer once something is attached. Defaults to "Change" plus the noun. */
  changeLabel: string;
}) {
   ...
   * each of the ones it replaced. Evidence can also be a PDF, which no {@code img} will draw,
  ...
  const pick = async (picked: File | undefined) => {
    ...
    try {
      ...
      // The compressed bytes are what gets sent, so the File handed upward is the compressed
      // one. Keeping the original here would mean compressing twice and sending whichever
      // copy the upload path happened to pick up.
  ...
  return (
    <Stack spacing={1}>
      ...
      {/*
        ...
        figures on the paper can actually be read — which is the whole reason it is attached.
      */}
